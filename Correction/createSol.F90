...
FUNCTION scalar_size(x, y, epsilon) RESULT(siz)
  ...
  REAL(KIND=8), DIMENSION(0:2)      :: hess
  REAL(KIND=8), DIMENSION(0:1)      :: lambda
  REAL(KIND=8), DIMENSION(0:1,0:1)  :: vp
  REAL(KIND=8)                      :: isqhmax
  REAL(KIND=8)                      :: hmax = 10. !maximal edge size
  INTEGER                           :: k

  !! Hessian computation
  CALL hessian(x,y,hess)
  DO k=0,2
     hess(k) =  hess(k) * 2./(9.*epsilon)
  ENDDO

  !! Eigenvalues/vectors
  CALL eigenvals(hess,lambda,vp)

  lambda(0) = dabs(lambda(0))
  lambda(1) = dabs(lambda(1))
  ...
  IF ( lambda(0) >= lambda(1) ) THEN
     siz = 1./sqrt(lambda(0))
  ELSE IF ( lambda(1) > lambda(0) ) THEN
     siz = 1./sqrt(lambda(1))
  ENDIF
...
SUBROUTINE tensor_size(x, y, epsilon, siz)
  ...
  !! Hessian computation
  CALL hessian(x,y,siz)
  DO k=0,2
     siz(k) =  siz(k) * 2./(9.*epsilon)
  ENDDO

  !! Eigenvalues/vectors
  CALL eigenvals(siz,lambda,vp)
